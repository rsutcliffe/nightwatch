...
@Test func brightNightNeverBlamesTheMoon() throws {
    ...
    let night = try Ephemeris.night(localDate: utc(2026, 7, 30, 12, 0), site: home)
    ...
    let plan = Planner.plan(night: night, forecast: Forecast(fetchedAt: start, latitude: home.latitude, longitude: home.longitude, hours: hours, seeingSource: nil),
                            catalog: Catalog(objects: []), constellations: [], site: home, fov: FieldOfView(widthDeg: 2.1, heightDeg: 1.2), rule: GoRule(), bright: b)
...
@Test func bezelWindowCrossingMidnight() {
    ...
    let s = Bezel.slots(darkness: dark, windows: [window], primary: window, hours: h, site: home)
    #expect(s.count == 60)
...
@Test func bezelWithoutDarknessIsAllDaylight() {
    #expect(Bezel.slots(darkness: nil, windows: [], primary: nil, hours: [], site: home) == Array(repeating: .daylight, count: 60))
}
```
...
private func plan(up: Double?, rise: Date? = nil, set: Date? = nil) throws -> NightPlan {
    let night = try Ephemeris.night(localDate: utc(2026, 11, 20, 12, 0), site: home)
...
@Test func moonTonightCases() throws {
    let n = try Ephemeris.night(localDate: utc(2026, 11, 20, 12, 0), site: home)
    ...
    #expect(Copy.moonText(.sets(inside), site: home) == "Sets \(Copy.hhmm(inside, site: home))")
}

@Test func planRecordsTheMoonsShareOfDarkness() throws {
    let night = try Ephemeris.night(localDate: utc(2026, 11, 20, 12, 0), site: home)
    let p = Planner.plan(night: night, forecast: Forecast(fetchedAt: night.sunset, latitude: home.latitude, longitude: home.longitude, hours: [], seeingSource: nil),
                         catalog: Catalog(objects: []), constellations: [], site: home, fov: FieldOfView(widthDeg: 2.1, heightDeg: 1.2), rule: GoRule())
...
@Test func brightPlanetsCarryARealMoonSeparation() throws {
    ...
    for h in 0..<8 {   // 21:00Z to 04:00Z, so at least one planet is up
        ...
        let moon = Ephemeris.moon(at: t, site: home).position
        for p in Planner.brightTargets(at: t, site: home) where p.id != "moon" {

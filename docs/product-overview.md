...
*As of 25 September 2026, version 0.6.10 "Night Watch". Free and open source, MIT licence. https://github.com/rsutcliffe/nightwatch*
...
| 0.6.10 | Night Watch, patch 10 | 25 September 2026 | macOS now shows its location prompt: the signed download lacked the location permission Apple requires of hardened-runtime apps, so the prompt never appeared and location had to be switched on in System Settings; "Use this Mac's location" also waits for the answer instead of reporting "not available" while the prompt is on screen; the notifications prompt comes after the welcome's "Start watching", not over it; the install steps say that macOS asks once before opening a downloaded app |

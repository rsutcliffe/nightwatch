...
**First launch.** A welcome window asks what you image with (the telescope presets, or something else) and where you observe from: "Use this Mac's location" or "Add a site…". Choosing this Mac's location brings up macOS's permission prompt; the welcome says "If macOS asks, choose Allow." and waits up to a minute. Closing the welcome ("Start watching") then brings up the notifications prompt, after the welcome has said what they are for; no alert is worked out until notifications are allowed. Before any of this, macOS itself asks once whether to open an app downloaded from the internet. Anyone already set up skips it: an existing settings file, or a forecast cached by an earlier version.

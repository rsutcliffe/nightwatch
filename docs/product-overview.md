...
- Caches under `~/Library/Caches/Nightwatch`: forecasts, plans and images, all rebuilt if a cleaner or macOS empties the folder. Which alerts tonight has already had (`alerts-state.json`, `aurora-state.json`) sits beside the settings instead, so emptying the caches never repeats a notification. Settings in `~/Library/Application Support/Nightwatch/config.json`, a plain JSON file which can be symlinked into iCloud Drive to share across Macs.
...
| 0.6.9 | Night Watch, patch 9 | 25 September 2026 | Emptying the app caches (a cleaner, or macOS when disk space is low) no longer repeats a notification: the record of which alerts tonight has already had moves beside the settings, and existing records move across on the first launch |

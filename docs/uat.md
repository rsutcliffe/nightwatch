...
51. After an alert, `~/Library/Application Support/Nightwatch` holds alerts-state.json and `~/Library/Caches/Nightwatch` does not. Quit, delete `~/Library/Caches/Nightwatch` (as a cleaner would), relaunch → no repeat notification and no welcome window. → pass.

...
47. A first launch with no settings opens "Welcome to Nightwatch": choosing a telescope sets the field of view; "Use this Mac's location" asks macOS for permission and, on Allow, fills in the location with no visit to System Settings (test on a Mac where Nightwatch has never had location access); the notifications prompt appears only after "Start watching"; "Add a site…" opens the sheet; "Start watching" closes it and it never returns. Nobody already set up sees it: an existing settings file, or a forecast cached by an earlier version. → pass.
